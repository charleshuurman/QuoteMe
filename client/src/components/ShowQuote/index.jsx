...
const ShowQuote = ({quote}) => {
  ...
  function QuoteAnalyzer({ quoteToAnalyze }) {
    ...
    if (analyzeResponse.loading) return <p>Loading...</p>;
    ...
    return (
      <div className="flex max-lg:w-full max-lg:justify-center">
        {analyzeResponse.data?.analyzeQuote && (
          <div className="chat chat-start">
            <div className="badge">{analyzeResponse.data.analyzeQuote.emotion}</div>
            <div className="chat-bubble">{analyzeResponse.data.analyzeQuote.content}</div>
          </div>
        )}
  ...
  return (
    <li
      key={quote._id}
      className="card flex flex-col md:flex-row bg-base-300 m-2 shadow-xl rounded-lg overflow-hidden"
                ...
                <>
                  {quote.isPrivate ? (
                    <button
                      ...
                      onClick={(event) => setPublicButton(quote._id, event)}
                      data-id={quote._id}
                    >
                      Share to Public
                    </button>
